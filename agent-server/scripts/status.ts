...
// Servicio de arranque automático — el mecanismo depende del sistema operativo
try {
  ...
  if (process.platform === 'darwin') {
    ...
    const output = execSync('systemctl --user is-active agent-server 2>/dev/null || echo "inactive"', { encoding: 'utf8' })
    check('systemd service', output.trim() === 'active', output.trim())
  } else {
    // Windows: pm2 es la vía documentada en el README
    const output = execSync('pm2 jlist 2>nul || echo []', { encoding: 'utf8', shell: 'cmd.exe' })
    const running = /"name"\s*:\s*"agent-server"/.test(output)
    check('servicio de inicio (pm2)', running, running ? 'activo' : 'no configurado — opcional, ver README')
  }
} catch {
  check('servicio de inicio', false, 'no configurado — opcional, ver README')
}
