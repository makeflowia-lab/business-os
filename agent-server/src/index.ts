...
async function main(): Promise<void> {
  ...
  // Iniciar scheduler (no siembra crons — usa scripts/seed-crons.ts para eso)
  initScheduler()
...
main().catch((err) => {
  // Causa más común en una instalación nueva: el token de Telegram sigue siendo
  // el de la plantilla o es inválido. Telegram responde 404 y grammY lanza un
  // error críptico ("Call to 'deleteWebhook' failed!"), así que lo traducimos.
  const msg = err instanceof Error ? err.message : String(err)
  if (/404: Not Found|401: Unauthorized|Unauthorized/i.test(msg)) {
    console.error(
      '\n✗ Telegram rechazó el token.\n' +
        '  Revisa TELEGRAM_BOT_TOKEN en agent-server/.env — si sigue siendo el valor\n' +
        "  de ejemplo ('your-bot-token'), crea un bot con @BotFather en Telegram,\n" +
        '  copia el token que te da y vuelve a arrancar.\n' +
        '  Diagnóstico completo: npm run status\n',
    )
  } else {
