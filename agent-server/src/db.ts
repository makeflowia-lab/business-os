...
export interface ScheduledTask {
  ...
  created_at: number
  /** Reintentos consecutivos por errores transitorios (cuota/límite/red). 0 = sin reintentos pendientes. */
  retry_count: number
}
...
/**
 * Migraciones idempotentes para BDs creadas antes de que existiera una columna.
 * Patrón: si la columna no existe en la tabla, se añade con ALTER TABLE.
 */
...
/** Corrida consumida (éxito o error definitivo): avanza al siguiente slot y limpia los reintentos. */
export function updateTaskAfterRun(id: string, result: string, nextRun: number): void {
  db.prepare(`
    UPDATE scheduled_tasks
    SET last_run = ?, last_result = ?, next_run = ?, retry_count = 0
    WHERE id = ?
  `).run(Math.floor(Date.now() / 1000), result, nextRun, id)
}

/**
 * Error transitorio: NO consume el slot del cron. Reprograma con backoff y
 * persiste el contador de reintentos.
 */
export function updateTaskAfterTransientError(
  id: string,
  result: string,
  nextRun: number,
  retryCount: number
  ...
  db.prepare(`
    UPDATE scheduled_tasks
    SET last_run = ?, last_result = ?, next_run = ?, retry_count = ?
    WHERE id = ?
  `).run(Math.floor(Date.now() / 1000), result, nextRun, retryCount, id)
}

/** Mueve el next_run sin marcar una corrida (anti-estampida al arrancar). */
export function rescheduleTask(id: string, nextRun: number): void {
